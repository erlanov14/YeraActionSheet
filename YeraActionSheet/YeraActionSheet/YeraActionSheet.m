...
#define YERAAS_SCREEN_WIDTH ([UIScreen mainScreen].bounds.size.width)
#define YERAAS_SCREEN_HEIGHT ([UIScreen mainScreen].bounds.size.height)
...
@implementation YeraActionSheet
- (void)dealloc {
...
- (instancetype)initWithTitle:(NSString *)title
    ...
    if (self) {
        ...
        UIView *maskView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, YERAAS_SCREEN_WIDTH, YERAAS_SCREEN_HEIGHT)];
        ...
        self.frame = CGRectMake(0, YERAAS_SCREEN_HEIGHT, YERAAS_SCREEN_WIDTH, 0);
        ...
        if (title) {
            ...
            UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, YERAAS_SCREEN_WIDTH, [self getSizeWithString:title font:kTitleFontSize].height + 2 * 15)];
        ...
        if (destructiveButtonTitle) {
            ...
            destructiveButton.frame = CGRectMake(0, actionSheetH, YERAAS_SCREEN_WIDTH, kRowHeight);
        ...
        if (otherButtonTitles.count) {
            ...
            for (NSString *otherButtonTitle in otherButtonTitles) {
                ...
                otherButton.frame = CGRectMake(0, actionSheetH, YERAAS_SCREEN_WIDTH, kRowHeight);
        ...
        if (cancelButtonTitle) {
            ...
            cancelButton.frame = CGRectMake(0, actionSheetH, YERAAS_SCREEN_WIDTH, kRowHeight);
            cancelButton.tag = 0;
...
- (void)hide {
    ...
    [UIView animateWithDuration:0.35f animations:^{
        ...
        CGRect frame = self.frame;
        frame.origin.y = YERAAS_SCREEN_HEIGHT;
        self.frame = frame;
...
- (void)show {
    ...
    [UIView animateWithDuration:0.35f animations:^{
        ...
        CGRect frame = self.frame;
        frame.origin.y = YERAAS_SCREEN_HEIGHT - self.frame.size.height;
        self.frame = frame;
    } completion:^(BOOL finished) {
...
+ (void)showWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle   destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles selectedBlock:(void (^)(NSInteger index))selectedBlock {
    ...
    YeraActionSheet *actionSheet = [[YeraActionSheet alloc] initWithTitle:title cancelButtonTitle:cancelButtonTitle destructiveButtonTitle:destructiveButtonTitle otherButtonTitles:otherButtonTitles selectedBlock:selectedBlock];
...
- (CGSize)getSizeWithString:(NSString *)string font:(UIFont *)font{
    ...
    return [string boundingRectWithSize:CGSizeMake(YERAAS_SCREEN_WIDTH, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: font} context:nil].size;
}

- (UIImage *)getImageWithColor:(UIColor *)color {
    
    CGSize imageSize = CGSizeMake(YERAAS_SCREEN_WIDTH, 1.0f);
